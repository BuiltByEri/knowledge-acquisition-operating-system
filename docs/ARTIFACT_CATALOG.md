...
| `artifact.knowledge_inventory` | Knowledge Inventory | `knowledge-inventory` | `artifacts/{SOURCE_CODE}_KNOWLEDGE_INVENTORY.md` | Baseline Reconciliation, or Source Certification for empty inventories |
...
| `artifact.implementation_approval` | Implementation Approval | `implementation-approval` | `artifacts/{SOURCE_CODE}_IMPLEMENTATION_APPROVAL.md` | Structured Data Implementation, or Post-Implementation Review for authorized no-change scope |
| `artifact.structured_data_implementation_report` | Structured Data Implementation Report | `structured-data-implementation` | `artifacts/{SOURCE_CODE}_STRUCTURED_DATA_IMPLEMENTATION_REPORT.md` | Post-Implementation Review when implementation occurs |
...
Empty inventories require Source Certification and Source Completion but do not require implementation artifacts. Intentionally skipped implementation requires Implementation Approval, Post-Implementation Review, Source Certification, and Source Completion, but does not require `artifact.structured_data_implementation_report`.
