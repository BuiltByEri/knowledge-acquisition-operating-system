...
kaos_skill:
  ...
  owner: "KAOS Core"
  description: "Review actual structured-data changes against authorization, validate data quality, and identify remediation without certifying the source."
  depends_on_skills: ["structured-data-implementation"]
  prerequisites:
    - { artifact_id: "artifact.structured_data_implementation_report", required: true }
  consumes:
...
## 1. Purpose
Review implemented changes against the approved plan before source certification.

## 2. When to Use
Use after Structured Data Implementation completes or records a skipped implementation.

## 3. Prerequisites
The implementation report must document actual changes, target locations, validation output, actual deltas, expected deltas, and remaining risks.

## 4. Inputs
Inputs include implementation approval, implementation report, changed structured data, validation output, source manifest, and upstream artifact references.

## 5. Required Reading
Read KAOS Core, Implementation Approval, Structured Data Implementation Report, and relevant upstream artifacts referenced by the implemented changes.

## 6. Repository Expectations
The repository contains the implemented state to review. This skill does not introduce new structured-data scope.

## 7. Execution Steps
Compare actual changes to authorization, verify excluded-scope protection, review changed records, verify relationships and references, review validation output, assign record-level review outcomes, identify in-scope remediation, identify unresolved risks, and produce the review artifact.

## 8. Validation Requirements
Validate record coverage, approved-scope adherence, actual-vs-expected deltas, data quality, reference integrity, validation results, remediation ownership, and certification readiness.

## 9. Success Criteria
Certification can evaluate a reviewed implementation state with known defects, notes, and unresolved risks.
...
Failure modes include reviewing only counts, missing excluded-scope contamination, approving unvalidated changes, and turning review into certification.

## 16. Artifacts Produced
`artifact.post_implementation_review` is produced only by this skill.

## 17. Exit Criteria
Exit when review outcomes, remediation needs, validation interpretation, risks, status, decision, blockers, and next permitted transition are documented.

## 18. Related Skills
Previous skill: `structured-data-implementation`. Next skill: `source-certification`.
